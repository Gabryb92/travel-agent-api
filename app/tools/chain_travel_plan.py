...
def chain_travel_plan(params: TravelPlanInput) -> TravelPlanOutput:
    ...
    #Si usa Pydantic per la validazione dei dati, questo assicura che tutti i parametri inseriti rispettino i formati richiesti e che i valori opzionali abbiano dei default sensati
    output_parser = PydanticOutputParser(pydantic_object=TravelPlanOutput)
    
    #Catena, i dati dell'utente vengono validati e inseriti in un prompt strutturato che viene successivamente elaborato dalla AI e la risposta è parsata in un formato strutturato per giornata.
